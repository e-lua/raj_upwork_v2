...
func Si_Find_Profile(symbol string) (models.CompanyProfile, error) {
	...
	q := `SELECT symbol,price,beta,volAvg,mktCap,lastDiv,rangeCompany,changesCompany,companyName,currency,cik,isin,cusip,exchange,exchangeShortName,industry,website,description,ceo,sector,country,fullTimeEmployees,phone,address,city,state,zip,dcfDiff,dcf,image,ipoDate,defaultImage,isEtf,isActivelyTrading,isAdr,isFund FROM CompanyProfile2 WHERE symbol = ?`
